---
id: pear-troubleshooting
title: Peer Troubleshooting
sidebar_label: Troubleshooting Peer
sidebar_position: 6
---
...
The `Pear.teardown(cb)` callback is triggered whenever the Pear app start to unload. If it is not exiting, then something is keeping the applications event loop running. A common cause of this is not cleaning up the [worker pipe](https://docs.pears.com/api#const-pipe-pear.worker.pipe) by calling `pipe.end()` to gracefully end the writable part of the stream.
...
2. If no helpful info, run sidecar with logs `pear sidecar --log-level 3`.\
   ...
   \&#xNAN;*Note* that this will close any running pear applications such as Keet.
...
* An application is trying to open the same storage twice.\
  If using `Corestore`, it is recommended to only create only one instance and reusing it.
...
* Not destroying the hyperswarm instance in the `Pear.teardown()` callback so Hyperswarm can unannounce and clean up the DHT.\
