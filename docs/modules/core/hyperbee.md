...
Hyperbee is an append only B-tree based on [`Hypercore`](https://docs.pears.com/building-blocks/hypercore). It provides a key/value-store API, with methods for inserting and getting key-value pairs, atomic batch insertions, and creating sorted iterators. It uses a single Hypercore for storage, using a technique called embedded indexing. It provides features like cache warmup extension, efficient diffing, version control, sorted iteration, and sparse downloading.
...
* [Hyperbee](https://docs.pears.com/building-blocks/hyperbee):
  ...
  * Basic:
    ...
    * Methods:
      * [db.ready()](#db.ready)
      * [db.close()](#db.close)
      * [db.put(key, \[value\], \[options\])](#db.put)
      * [db.get(key, \[options\])](#db.get)
      * [db.del(key, \[options\])](#db.del)
      * [db.getBySeq(seq, \[options\])](#db.getbyseq)
      * [db.replicate(isInitiatorOrStream)](#db.replicate)
      * [db.batch()](#db.batch)
        * [batch.put(key, \[value\], \[options\])](#batch.put)
        * [batch.get(key, \[options\])](#batch.get)
        * [batch.del(key, \[options\])](#batch.del)
        * [batch.flush()](#batch.flush)
        * [batch.close()](#batch.close)
      * [db.createReadStream(\[range\], \[options\])](#db.createreadstream)
      * [db.peek(\[range\], \[options\])](#db.peek)
      * [db.createHistoryStream(\[options\])](#db.createhistorystream)
      * [db.createDiffStream(otherVersion, \[options\])](#db.creatediffstream)
      * [db.getAndWatch(key, \[options\])](#db.getandwatch)
      * [db.watch(\[range\])](#db.watch)
      * [db.checkout(version)](#db.checkout)
      * [db.snapshot()](#db.snapshot)
      * [db.sub('sub-prefix', \[options\])](#db.sub)
      * [db.getHeader(\[options\])](#db.getheader)
      * [Hyperbee.isHyperbee(core, \[options\])](#db.ishyperbee)
...
Make a new Hyperbee instance. `core` should be a [`Hypercore`](https://docs.pears.com/building-blocks/hypercore).
...
#### **`db.core`** <a href="#db.core" id="db.core"></a>

The underlying [Hypercore](https://docs.pears.com/building-blocks/hypercore) backing this bee.

#### **`db.version`** <a href="#db.version" id="db.version"></a>

A number that indicates how many modifications were made, is useful as a version identifier.

#### **`db.id`** <a href="#db.id" id="db.id"></a>

String containing the ID (z-base-32 of the public key) identifying this bee.

#### **`db.key`** <a href="#db.key" id="db.key"></a>

Buffer containing the public key identifying this bee.

#### **`db.discoveryKey`** <a href="#db.discoverykey" id="db.discoverykey"></a>
...
#### **`await db.close()`** <a href="#db.close" id="db.close"></a>

Fully close this bee, including its core.

#### **`await db.put(key, [value], [options])`** <a href="#db.put" id="db.put"></a>
...
#### **`const { seq, key, value } = await db.get(key, [options])`** <a href="#db.get" id="db.get"></a>
...
#### **`await db.del(key, [options])`** <a href="#db.del" id="db.del"></a>
...
#### **`const { key, value } = await db.getBySeq(seq, [options])`** <a href="#db.getbyseq" id="db.getbyseq"></a>

Gets the key and value from a block number.

`seq` is the Hypercore index. Returns `null` if block doesn't exists.

#### **`const stream = db.replicate(isInitiatorOrStream)`** <a href="#db.replicate" id="db.replicate"></a>

See more about how replicate works at [core.replicate](https://docs.pears.com/hypercore#const-stream-core.replicate-isinitiatororreplicationstream).

#### **`const batch = db.batch()`** <a href="#db.batch" id="db.batch"></a>
...
#### **`await batch.put(key, [value], [options])`** <a href="#db.put" id="db.put"></a>

Inserts a key into a batch.

`options` are the same as **`db.put`** method.

#### **`const { seq, key, value } = await batch.get(key, [options])`** <a href="#batch.get" id="batch.get"></a>

Gets a key, and value out of a batch.

`options` are the same as **`db.get`** method.

#### **`await batch.del(key, [options])`** <a href="#batch.del" id="batch.del"></a>

Deletes a key into the batch.

`options` are the same as **`db.del`** method.

#### **`await batch.flush()`** <a href="#batch.flush" id="batch.flush"></a>

Commits the batch to the database, and releases any locks it has acquired.

#### **`await batch.close()`** <a href="#batch.close" id="batch.close"></a>
...
#### **`const stream = db.createReadStream([range], [options])`** <a href="#db.createreadstream" id="db.createreadstream"></a>
...
#### **`const { seq, key, value } = await db.peek([range], [options])`** <a href="#db.peek" id="db.peek"></a>

Similar to doing a read stream and returning the first value, but a bit faster than that.

#### **`const stream = db.createHistoryStream([options])`** <a href="#db.createhistorystream" id="db.createhistorystream"></a>
...
#### **`const stream = db.createDiffStream(otherVersion, [options])`** <a href="#db.creatediffstream" id="db.creatediffstream"></a>
...
#### `const entryWatcher = await db.getAndWatch(key, [options])` <a href="#db.getandwatch" id="db.getandwatch"></a>
...
#### **`const watcher = db.watch([range])`** <a href="#db.watch" id="db.watch"></a>
...
#### **`const snapshot = db.checkout(version)`** <a href="#db.checkout" id="db.checkout"></a>

Get a read-only snapshot of a previous version.

#### **`const snapshot = db.snapshot()`** <a href="#db.snapshot" id="db.snapshot"></a>

Shorthand for getting a checkout for the current version.

#### **`const sub = db.sub('sub-prefix', options = {})`** <a href="#db.sub" id="db.sub"></a>
...
#### **`const header = await db.getHeader([options])`** <a href="#db.getheader" id="db.getheader"></a>
...
#### **`const isHyperbee = await Hyperbee.isHyperbee(core, [options])`** <a href="#db.ishyperbee" id="db.ishyperbee"></a>
