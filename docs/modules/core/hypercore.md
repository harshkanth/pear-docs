...
* A [`truncate`](#await-core.truncate-newlength-forkid) method for intentionally creating a new fork, starting at a given length. We use this method extensively in [`autobase`](https://docs.pears.com/building-blocks/autobase).

> [GitHub (Hypercore)](https://github.com/holepunchto/hypercore)

* [Hypercore](https://docs.pears.com/building-blocks/hypercore)
  * [Creating a new instance](#installation)
  * Basic:
    * Properties:
    ...
    * Methods:
      * [core.append(block)](#core.append)
      * [core.get(index, \[options\])](#core.get)
      * [core.has(start, \[end\])](#core.has)
      * [core.update()](#core.update)
      * [core.seek(byteOffset)](#core.seek)
      * [core.createReadStream(\[options\])](#core.createreadstream)
      * [core.createByteStream(\[options\])](#core.createbytestream)
      * [core.clear(start, \[end\], \[options\])](#core.clear)
      * [core.truncate(newLength, \[forkId\])](#core.truncate)
      * [core.purge()](#core.purge)
      * [core.treeHash(\[length\])](#core.treehash)
      * [core.download(\[range\])](#core.download)
      * [core.session(\[options\])](#core.session)
      * [core.info(\[options\])](#core.info)
      * [core.close()](#core.close)
      * [core.ready()](#core.ready)
      * [core.replicate(isInitiatorOrReplicationStream, \[options\])](#core.replicate)
      * [core.findingPeers()](#core.findingpeers)
      * [core.session(\[options\])](#core.session)
      * [core.snapshot(\[options\])](#core.snapshot)
    * Events:
      * [append](#core.onappend)
      * [truncate](#core.ontruncate)
      * [ready](#core.onready)
      * [close](#core.onclose)
      * [peer-add](#core.onpeer-add)
      * [peer-remove](#core.onpeer-remove)
...
#### **`core.readable`** <a href="#core.readable" id="core.readable"></a>

Can we read from this core? After [closing](#await-coreclose) the core this will be `false`.

#### **`core.id`** <a href="#core.id" id="core.id"></a>

A string containing the ID (z-base-32 of the public key) that identifies this core.

#### **`core.key`** <a href="#core.key" id="core.key"></a>
...
#### **`core.keyPair`** <a href="#core.keypair" id="core.keypair"></a>

An object containing buffers of the core's public and secret key

#### **`core.discoveryKey`** <a href="#core.discoverykey" id="core.discoverykey"></a>
...
> The above properties are populated after [`ready`](#await-core.ready) has been emitted. Will be `null` before the event.

#### **`core.encryptionKey`** <a href="#core.encryptionkey" id="core.encryptionkey"></a>

Buffer containing the optional block encryption key of this core. Will be `null` unless block encryption is enabled.

#### **`core.writable`** <a href="#core.writable" id="core.writable"></a>

Can we append to this core?

> Populated after [`ready`](#await-core.ready) has been emitted. Will be `false` before the event.

#### **`core.length`** <a href="#core.length" id="core.length"></a>

The number of blocks of data available on this core. If `sparse: false`, this will equal `core.contiguousLength`.

#### **`core.contiguousLength`** <a href="#core.contiguouslength" id="core.contiguouslength"></a>

The number of blocks contiguously available starting from the first block of this core.

#### **`core.fork`** <a href="#core.fork" id="core.fork"></a>

The current fork id of this core

> The above properties are populated after [`ready`](#await-core.ready) has been emitted. Will be `0` before the event.

#### **`core.padding`** <a href="#core.padding" id="core.padding"></a>

The amount of padding applied to each block of this core. Will be `0` unless block encryption is enabled.

#### Methods

#### **`const { length, byteLength } = await core.append(block)`** <a href="#core.append" id="core.append"></a>
...
#### **`const block = await core.get(index, [options])`** <a href="#core.get" id="core.get"></a>
...
#### **`const has = await core.has(start, [end])`** <a href="#core.has" id="core.has"></a>

Check if the core has all blocks between `start` and `end`.

#### **`const updated = await core.update([options])`** <a href="#core.update" id="core.update"></a>
...
#### **`const [index, relativeOffset] = await core.seek(byteOffset, [options])`** <a href="#core.seek" id="core.seek"></a>
...
#### **`const stream = core.createReadStream([options])`** <a href="#core.readstream" id="core.readstream"></a>
...
#### `const bs = core.createByteStream([options])` <a href="#core.createbytestream" id="core.createbytestream"></a>
...
#### **`const cleared = await core.clear(start, [end], [options])`** <a href="#core.clear" id="core.clear"></a>
...
#### **`await core.truncate(newLength, [forkId])`** <a href="#core.truncate" id="core.truncate"></a>
...
#### `await core.purge()` <a href="#core.purge" id="core.purge"></a>

Purge the Hypercore from storage, completely removing all data.

#### **`const hash = await core.treeHash([length])`** <a href="#core.purge" id="core.purge"></a>

Get the Merkle Tree hash of the core at a given length, defaulting to the current length of the core.

#### **`const range = core.download([range])`** <a href="#core.download" id="core.download"></a>
...
#### **`const session = await core.session([options])`** <a href="#core.session" id="core.session"></a>
...
#### **`const info = await core.info([options])`** <a href="#core.info" id="core.info"></a>
...
#### **`await core.close()`** <a href="#core.close" id="core.close"></a>

Close this core and release any underlying resources.

#### **`await core.ready()`** <a href="#core.ready" id="core.ready"></a>
...
#### **`const stream = core.replicate(isInitiator|stream, options)`** <a href="#core.replicate" id="core.replicate"></a>
...
To replicate a Hypercore using [`Hyperswarm`](https://docs.pears.com/building-blocks/hyperswarm):
...
> To replicate many Hypercores over a single Hyperswarm connection, see [`Corestore`](https://docs.pears.com/helpers/corestore).

If not using [`Hyperswarm`](https://docs.pears.com/building-blocks/hyperswarm) or [`Corestore`](https://docs.pears.com/helpers/corestore), specify the `isInitiator` field, which will create a fresh protocol stream that can be piped over any transport:
...
#### **`const done = core.findingPeers()`** <a href="#core.findingpeers" id="core.findingpeers"></a>
...
#### **`core.snapshot([options])`** <a href="#core.snapshot" id="core.snapshot"></a>
...
If [`core.update()`](#const-updated--await-coreupdateoptions) is explicitly called on the snapshot instance, it will no longer be locked to the previous data. Rather, it will get updated with the current state of the Hypercore instance.

`options` are the same as the options to [`core.session()`](#coresessionoptions).
...
#### **`core.on('append')`** <a href="#core.onappend" id="core.onappend"></a>

Emitted when the core has been appended to (i.e., has a new length/byte length), either locally or remotely.

#### **`core.on('truncate', ancestors, forkId)`** <a href="#core.ontruncate" id="core.ontruncate"></a>

Emitted when the core has been truncated, either locally or remotely.

#### **`core.on('ready')`** <a href="#core.onready" id="core.onready"></a>

Emitted after the core has initially opened all its internal state.

#### **`core.on('close')`** <a href="#core.onclose" id="core.onclose"></a>

Emitted when the core has been fully closed.

#### **`core.on('peer-add')`** <a href="#core.onpeer-add" id="core.onpeer-add"></a>

Emitted when a new connection has been established with a peer.

#### **`core.on('peer-remove')`** <a href="#core.onpeer-remove" id="core.onpeer-remove"></a>
